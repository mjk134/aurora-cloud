...
export async function loginAction(prevState: any, data: FormData): Promise<FormActionResponse | void> {
    ...
    if (!user) {
        return { error: true, message: "No user with this username, please sign up.", location: "username", values: { username, password } };
    }
    
    if (password.length < 8 || password.length > 20) {
        return { error: true, message: "Invalid password.", location: "password", values: { username, password } };
    }
